...
val cryptoDataModule = module {
    ...
    singleOf(::CoinsRemoteMediator)
    single<CoinRepository> {
        CoinRepositoryImp(
            pager = get(named("coinPager")),
            coinRemoteDataSource = get(),
            coinLocalDataSource = get()
        )
    }
    singleOf(::GetPagedCoinsUseCase)
    singleOf(::GetCoinHistoryUseCase)
    singleOf(::GetFavoriteCoinIdsUseCase)
    singleOf(::SetCoinFavoriteUseCase)
    single(named("coinPagingConfig")) { PagingConfig(pageSize = 50, initialLoadSize = 50) }
    single(named("coinPager")) {
        Pager(
            config = get(named("coinPagingConfig")),
            remoteMediator = get<CoinsRemoteMediator>(),
            pagingSourceFactory = {
